...
class Memo
  ...
  def update(params)
    ...
    @@conn.exec_params('UPDATE memos SET title = $1::text, content = $2::text WHERE id = $3::int;', [params[:title], params[:content], @id.to_i])
  ...
  def self.find(memo_id)
    res = @@conn.exec_params('SELECT * FROM memos WHERE id = $1::int', [memo_id.to_i]).first
  ...
  def destroy
    ...
    @@conn.exec_params('DELETE FROM memos WHERE id = $1::int', [@id.to_i])
    target
  end
end
